...
public class Map
{
    ...
    public static int ScreenWidth => Screen.PrimaryScreen?.Bounds.Width ?? DisplaySize.X;
    public static int ScreenHeight => Screen.PrimaryScreen?.Bounds.Height ?? DisplaySize.Y;
    ...
    public static void LoadMap(string ID)
    {
        ...
        foreach (var i in Wz.EquipImageLib)
            i.Value.Dispose();
        ...
        foreach (var Iter in Map.Img.GetNode("info").Nodes)
            Map.Info.AddOrReplace(Iter.Text, Iter.ToInt());

        Map.Info.AddOrReplace("MapWidth", Map.Img.GetValue2("miniMap/width", 0));
        Map.Info.AddOrReplace("MapHeight", Map.Img.GetValue2("miniMap/height", 0));
        Map.Info.AddOrReplace("centerX", Map.Img.GetValue2("miniMap/centerX", DisplaySize.X / 2));
        Map.Info.AddOrReplace("centerY", Map.Img.GetValue2("miniMap/centerY", DisplaySize.Y / 2));
