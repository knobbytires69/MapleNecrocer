    ...
    public static Wz_Node GetNode(string Path)
    {
        ...
        if (Node != null)
        {
            if (Node.Value is Wz_Uol)
            {
                ...
                if (Node2 != null && Node2.FindNodeByPath("_outlink", true) != null)
                {
                    ...
                    switch (Split[0])
                    {
                        ...
                        case "Skill":
                            ...
                            break;
                        default:
                            var nodeWzFile = Node.GetNodeWzFile();
                            if (nodeWzFile != null)
                                DestPath = Regex.Replace(LinkStr, nodeWzFile.Type.ToString() + "/", "");
                            break;
                    };
                }
                return Node2;
...
internal class WzDict
{
    ...
    {
        if (Wz.EquipData.TryGetValue(Path, out var node))
